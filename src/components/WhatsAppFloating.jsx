...
export default function FloatingWhatsApp() {
  ...
  return (
    ...
    className={`fixed z-50 bottom-8 right-8 flex items-center justify-center w-16 h-16 rounded-full 
        bg-gradient-to-br from-green-400 to-green-600
        shadow-xl transition-all duration-700 font-sans
        ${visible ? "opacity-100 translate-y-0 animate-bounce-in animate-float" : "opacity-0 translate-y-8 pointer-events-none"}
        group
      `}
      style={{ boxShadow: "0 8px 32px 0 rgba(34,197,94,0.25)" }}
    >
{/* <div className="absolute inset-0 bg-success-500 rounded-2xl animate-ping opacity-20"></div> */}
      {/* <span className="absolute inset-0 rounded-full bg-white opacity-1 0 blur-xl animate-pulse-soft"></span> */}
  
  {/* Beautiful Pulse Rings */}
<span className="absolute inset-0 rounded-full animate-pulse-ring-1"></span>
<span className="absolute inset-0 rounded-full animate-pulse-ring-2"></span>
  ...
  viewBox="0 0 24 24"
  fill="currentColor"
className="text-white transition-transform duration-300 group-hover:scale-110 animate-icon-breathe"
...
.animate-icon-breathe {
  ...
  .animate-bounce-in {
    animation: bounce-in 0.9s cubic-bezier(0.23, 1, 0.32, 1);
  }
